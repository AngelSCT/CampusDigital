        ...
        <div class="space-y-6">
            ...
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="mt-1 text-sm text-white">Administra los roles y sus permisos</p>
                </div>
                <a :href="route('admin.roles.create')" class="inline-flex items-center px-4 py-2 border border-transparent shadow-lg shadow-blue-500/30 text-sm font-medium rounded-lg text-white bg-gradient-to-br from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 transition-all duration-200">
            ...
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 shadow-xl shadow-blue-500/10 rounded-xl border border-blue-500/20 overflow-hidden">
                    ...
                    <table class="min-w-full divide-y divide-slate-700">
                        <thead class="bg-slate-900/50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider whitespace-nowrap">Nombre</th>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider whitespace-nowrap">Usuarios</th>
                        ...
                        <tbody class="bg-slate-800/30 divide-y divide-slate-700">
                            <tr v-for="rol in roles.data" :key="rol.id" class="hover:bg-slate-700/30 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ rol.id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-white capitalize">{{ rol.nombre }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-slate-300 max-w-md truncate">{{ rol.descripcion || 'Sin descripción' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span v-if="rol.activo" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-500/20 text-green-400 border border-green-500/30">
                                        Activo
                                    </span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end gap-2 min-w-[100px]">
                                        ...
                                        <span v-else class="text-slate-600" title="Rol del sistema - No se puede eliminar">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div v-if="roles.data.length > 0" class="bg-slate-900/50 px-4 py-3 border-t border-slate-700 sm:px-6">
                    <div class="flex items-center justify-between">
                        <div class="flex-1 flex justify-between sm:hidden">
                            <a v-if="roles.prev_page_url" :href="roles.prev_page_url" class="relative inline-flex items-center px-4 py-2 border border-slate-600 text-sm font-medium rounded-lg text-white bg-slate-700 hover:bg-slate-600 transition-colors duration-200">
                                Anterior
                            </a>
                            <a v-if="roles.next_page_url" :href="roles.next_page_url" class="ml-3 relative inline-flex items-center px-4 py-2 border border-slate-600 text-sm font-medium rounded-lg text-white bg-slate-700 hover:bg-slate-600 transition-colors duration-200">
                                Siguiente
                            </a>
                        </div>
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-slate-300">
                                    Mostrando <span class="font-medium text-white">{{ roles.from }}</span> a <span class="font-medium text-white">{{ roles.to }}</span> de <span class="font-medium text-white">{{ roles.total }}</span> resultados
                                </p>
                            </div>
                            <div>
                                <nav class="relative z-0 inline-flex rounded-lg shadow-sm -space-x-px">
                                    <a v-for="link in roles.links" :key="link.label" :href="link.url" v-html="link.label" :class="[
                ...
                <div v-if="roles.data.length === 0" class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-white">No hay roles</h3>
                    <p class="mt-1 text-sm text-slate-400">Comienza creando un nuevo rol.</p>
                </div>
            </div>
        </div>
    </AuthLayout>
...
function applyFilters() {
    router.get(route('admin.roles.index'), filterForm, { preserveState: true });
}

function clearFilters() {
    filterForm.search = '';
    router.get(route('admin.roles.index'), {}, { preserveState: true });
}
