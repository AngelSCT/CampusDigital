        ...
        <div class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 shadow-xl shadow-blue-500/10 rounded-xl p-6 border border-blue-500/20">
                <form @submit.prevent="applyFilters" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <!-- Acción -->
                    <div>
                        <label class="block text-sm font-medium text-white mb-2">Acción</label>
                        <select 
                            v-model="form.accion"
                            class="w-full px-3 py-2 bg-slate-900 border border-slate-700 text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                        >
            ...
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 shadow-xl shadow-blue-500/10 rounded-xl overflow-hidden border border-blue-500/20">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-700">
                        ...
                        <tbody class="divide-y divide-slate-700">
                            <tr v-for="actividad in actividades.data" :key="actividad.id" class="hover:bg-slate-700/50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white">
                                    {{ actividad.id }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white">
                                    {{ actividad.usuario?.email || 'N/A' }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span 
                                        :class="actividad.exito ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'" 
                ...
                <!-- Paginación -->
                <div v-if="actividades.data.length > 0" class="bg-slate-900 px-4 py-3 border-t border-slate-700 sm:px-6">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-white">
                            Mostrando 
                            <span class="font-medium text-blue-400">{{ actividades.from }}</span>
                            a 
                            <span class="font-medium text-blue-400">{{ actividades.to }}</span>
                            de 
                            <span class="font-medium text-blue-400">{{ actividades.total }}</span>
                            resultados
                        </div>
        ...
        <div v-if="showModal" class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50" @click="showModal = false">
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 max-w-2xl w-full mx-4 border border-blue-500/20 shadow-2xl shadow-blue-500/20" @click.stop>
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-bold text-white">Detalles de la Actividad</h3>
                ...
                <div v-if="selectedActividad" class="space-y-3">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-sm font-medium text-slate-400">ID</p>
                            <p class="text-sm text-white">{{ selectedActividad.id }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-slate-400">Usuario</p>
                            <p class="text-sm text-white">{{ selectedActividad.usuario?.email || 'N/A' }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-slate-400">Módulo</p>
                            <p class="text-sm text-white">{{ selectedActividad.modulo }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-slate-400">Acción</p>
                            <p class="text-sm text-white">{{ selectedActividad.accion }}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-slate-400">Estado</p>
                            <span 
                                :class="selectedActividad.exito ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'" 
                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full border"
                            >
                                {{ selectedActividad.exito ? 'Éxito' : 'Fallido' }}
                            </span>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-slate-400">IP</p>
                            <p class="text-sm text-white">{{ selectedActividad.ip || 'N/A' }}</p>
                        </div>
                        <div v-if="selectedActividad.target_tabla">
                            <p class="text-sm font-medium text-slate-400">Tabla Afectada</p>
                            <p class="text-sm text-white">{{ selectedActividad.target_tabla }}</p>
                        </div>
                        <div v-if="selectedActividad.target_id">
                            <p class="text-sm font-medium text-slate-400">ID Afectado</p>
                            <p class="text-sm text-white">{{ selectedActividad.target_id }}</p>
                        </div>
                        <div class="col-span-2">
                            <p class="text-sm font-medium text-slate-400">Fecha</p>
                            <p class="text-sm text-white">{{ formatDate(selectedActividad.created_at) }}</p>
                        </div>
                        <div class="col-span-2">
                            <p class="text-sm font-medium text-slate-400">User Agent</p>
                            <p class="text-sm text-white break-all">{{ selectedActividad.user_agent || 'N/A' }}</p>
                        </div>
                        <div class="col-span-2">
                            <p class="text-sm font-medium text-slate-400">Detalle</p>
                            <p class="text-sm text-white">{{ selectedActividad.detalle || 'Sin detalles' }}</p>
                        </div>
                        <div v-if="selectedActividad.meta_json" class="col-span-2">
                            <p class="text-sm font-medium text-slate-400 mb-2">Metadata</p>
                            <pre class="text-xs text-white bg-slate-900 p-3 rounded-lg border border-slate-700 overflow-x-auto">{{ JSON.stringify(selectedActividad.meta_json, null, 2) }}</pre>
                        </div>
                    </div>
                </div>
            </div>
...
function showDetails(actividad) {
