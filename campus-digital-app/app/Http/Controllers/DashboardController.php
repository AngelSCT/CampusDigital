...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($user->hasRole('proveedor_area')) {
    ...
    private function dashboardAdministrador($user)
    {
        ...
        $pedidosConTarjeta = DB::table('pedido')
            ->whereNull('deleted_at')
            ->where('created_at', '>=', $hace30d)
            ->where('confirmado_con_tarjeta', true)
            ->count();

        $pedidosSinTarjeta = DB::table('pedido')
            ->whereNull('deleted_at')
            ->where('created_at', '>=', $hace30d)
            ->where('confirmado_con_tarjeta', false)
            ->count();

        $ingresosEntregados = DB::table('pedido')
            ->whereNull('deleted_at')
        ...
        $usuariosSinTarjeta = DB::table('usuario as u')
            ->leftJoin('tarjeta_universitaria as t', fn ($j) => $j->on('t.usuario_id', '=', 'u.id')->whereNull('t.deleted_at'))
            ->whereNull('u.deleted_at')
            ->whereNull('t.id')
            ->count();

        $tarjetasEsteMes = DB::table('tarjeta_universitaria')
            ->whereNull('deleted_at')
        ...
        return Inertia::render('Dashboard/Admin', [
            ...
            // Estas dos colecciones estaban rompiendo por movimientos.id_catalogo.
            'top' => collect(),
            'categorias' => collect(),
        ]);
    }
    ...
    private function dashboardProveedor($user)
    {
        ...
        $productosCount = \App\Models\Producto::where('tienda', $tienda->nombre)->count();

        $repartidoresCount = Usuario::where('tienda_id', $tienda->id)
    ...
    private function dashboardEstudiante($user)
    {
        ...
        $gastoPromedio = round((float) (PedidoTienda::where('usuario_id', $user->id)->avg('total') ?? 0), 2);
