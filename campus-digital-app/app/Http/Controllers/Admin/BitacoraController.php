...
class BitacoraController extends Controller
{
    ...
    public function exportAccesosPeriodo(Request $request)
    {
        ...
        Log::channel('daily')->info('BitacoraController@exportAccesosPeriodo: Exportación CSV de accesos por período.', [
            ...
            'filtros'     => $request->only(['evento', 'exito']),
        ]);
        ...
        $callback = function() use ($accesos, $request) {
            ...
            fputcsv($file, [
                'ID', 'Usuario', 'Email Intentado', 'Evento',
                'Estado', 'IP', 'User Agent', 'Detalle', 'Fecha y Hora',
            ]);
    ...
    public function exportActividadPeriodo(Request $request)
    {
        ...
        Log::channel('daily')->info('BitacoraController@exportActividadPeriodo: Exportación CSV de actividad por período.', [
            ...
            'fecha_desde' => $request->fecha_desde,
            'fecha_hasta' => $request->fecha_hasta,
            'filtros'     => $request->only(['modulo', 'accion']),
        ]);
        ...
        $callback = function() use ($actividades, $request) {
            ...
            fputcsv($file, [
                'ID', 'Usuario', 'Módulo', 'Acción', 'Estado',
                'IP', 'Tabla Afectada', 'ID Afectado', 'Detalle', 'User Agent', 'Fecha y Hora',
            ]);
    ...
    public function exportActividadModulo(Request $request)
    {
        ...
        $callback = function() use ($modulos, $request, $actividades) {
            ...
            foreach ($modulos as $modulo => $items) {
                ...
                fputcsv($file, ['ID', 'Usuario', 'Acción', 'Estado', 'IP', 'Detalle', 'Fecha y Hora']);
