<template>
    <div class="crud-theme">
        <div class="crud-shell max-w-5xl">
            <h1 class="crud-title mb-1">Editar catalogo unificado</h1>
            <p class="crud-subtitle mb-6">Actualiza datos base y modulos relacionados desde una sola pantalla.</p>

            <div class="draft-notice">
                <span>
                    {{ draftRecovered ? 'Se recupero un borrador local.' : 'Autoguardado local activo para esta edicion.' }}
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <section class="crud-card">
                    ...
                    <div class="grid md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <p class="crud-muted mt-1">Si cambias el valor, se registra un nuevo precio historico.</p>
                        </div>
                        <div>
                ...
                <div class="flex items-center gap-2">
                    <button type="button" @click="restoreOriginalValues" class="crud-btn-secondary">
...
const initialValues = {
    ...
    areas: props.catalogo.areas ? props.catalogo.areas.map((a) => a.id_area) : [],
...
const DRAFT_KEY = `catalogo-edit-draft-${props.catalogo.id_catalogo}-v1`;
...
function submit() {
    form.put(`/catalogo/${props.catalogo.id_catalogo}`, {
        onSuccess: () => {
            clearDraft();
        }
    });
...
function loadDraft() {
    ...
    try {
        ...
        const shouldRestore = confirm('Se encontro un borrador local para esta edicion. Deseas restaurarlo?');
        if (!shouldRestore) {
...
function restoreOriginalValues() {
    const confirmed = confirm('Se restauraran los valores originales del registro. Deseas continuar?');
    if (!confirmed) {
        return;
    }
