    ...
    <AuthLayout>
        <div class="max-w-3xl mx-auto space-y-6">
            <div>
                ...
                <p class="mt-1 text-sm text-white">Modifica los datos del usuario</p>
            </div>
            ...
            <form @submit.prevent="submit" class="bg-gradient-to-br from-slate-800 to-slate-900 shadow-xl shadow-blue-500/10 rounded-xl border border-blue-500/20 p-6 space-y-6">
                ...
                <!-- Contraseña (Opcional) -->
                <div class="border-b border-slate-700 pb-4">
                    <h3 class="text-lg font-medium text-white">Cambiar Contraseña (Opcional)</h3>
                    <p class="mt-1 text-sm text-slate-400">Deja en blanco si no deseas cambiar la contraseña</p>
                </div>
...
function submit() {
    ...
    router.put(route('admin.usuarios.update', props.usuario.id), form, {
        onFinish: () => {
